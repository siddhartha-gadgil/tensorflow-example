...
object SVM {
  val dataArr = os.read
    ...
    val data = dataArr.map(arr => (arr(0), (arr(1), arr(2))))

    def run() = 
        Using(new Graph()){
            graph => 
            println("running SVM")
                val svm = Try(new SVM(graph, 0.1f, 1f)).fold(
                    err => {
                        println(err)
...
class SVM(graph: Graph, learningRate: Float, svmC: Float) {
  ...
  val w = tf.variable(tf.constant(Array(Array(0.1f, 0.1f))))
  ...
  tf.math.mul(
    tf.constant(0.5f),
    tf.reduceSum(tf.math.squaredDifference(w, tf.zerosLike(w)), tf.constant(Array(0,1)))
  )
  ...
  def fit(xy: Seq[(Float, (Float, Float))]) = Using(new Session(graph)) {
    session =>
      ...
      println("ran fit")
      println(Try(opLookup(b, session)))
